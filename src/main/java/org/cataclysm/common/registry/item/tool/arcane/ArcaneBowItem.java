package org.cataclysm.common.registry.item.tool.arcane;
...
import org.cataclysm.common.util.EnchantmentUtils;
import org.cataclysm.common.util.SoundUtils;

import java.util.ArrayList;
import java.util.List;

public class ArcaneBowItem extends BowItem {
    public ArcaneBowItem(Settings settings) {
        super(settings);
    }
    ...
    public void onStoppedUsing(ItemStack bow, World world, LivingEntity user, int remainingUseTicks) {
        ...
        SoundUtils.playSound(world, user, SoundEvents.BLOCK_AMETHYST_BLOCK_BREAK, 1.0F, 1.5F, 2.0F);
        SoundUtils.playSound(world, user, SoundEvents.ENTITY_ARROW_SHOOT, 0.8F, 0.9F, 1.1F);
    }
    ...
    private ItemStack takeAmmo(ItemStack bow, ItemStack ammo, LivingEntity shooter, ServerWorld server) {
        ...
        int infinityLevel = EnchantmentUtils.getEnchantmentLevel(server, bow, Enchantments.INFINITY);
